...
function ganjilGenapAsianGames(date, data) {
  //code here
  let pelanggaran = 0;
  if (!Number.isInteger(date) || date < 1 || date > 31) return "Invalid Date";

  data.forEach((vehicle) => {
    let [regionCode, registrationNumber, suffixCode] = vehicle.plat.split(" ");
    let lastDigit = registrationNumber[registrationNumber.length - 1];
